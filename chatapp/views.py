...
def private_chat_room_view(request, *args, **kwargs):
    ...
    if not user.is_authenticated:
        ...
        context = {}
        if room_id == None: #applicable to all not showing the room id in url
            #use filter for admin room query
            if group == 'admin':
                try:
            ...
            else: #use get for customer room query
                try:
                    ...
                    user = Account.objects.get(id=user.id) #user id is created
                    if user in admin_friendlist.friends.all():
                        # create a chatroom with admin
                        chat = find_or_create_private_chat(user, admin_account) #chat room id created
                        ...
                        pass
        else: #will be used by admin, to select each cutmer on the chat
